...
import CVPreviewModal from "../../resume/preview/CVPreviewModal";
...
function AIChatPanel() {
  ...
  const steps: ScriptedStep[] = [
    ...
    {
      question:
        'What\'s your current job title and location? (e.g. "Senior Product Designer, Cairo, Egypt")',
      apply: (answer) => {
    ...
    {
      question:
        "Tell me about your most recent job: company, title, and 1-2 key achievements.",
      apply: (answer) => {
        addExperience({
          company: "",
          jobTitle: "",
    ...
    },
    {
      ...
      apply: (answer) => {
        ...
        addEducation({
          university: answer.replace(yearMatch?.[0] ?? "", "").trim(),
          yearsOfGraduation: yearMatch?.[0] ?? "",
        });
      },
    },
    {
  ...
  async function handleSend() {
    ...
    if (nextIndex < steps.length) {
      setMessages((prev) => [
        ...
        { role: "assistant", content: steps[nextIndex].question },
      ]);
      ...
      setMessages((prev) => [
        ...
        { role: "assistant", content: reply },
        {
          role: "assistant",
          content:
            "That's everything I need! I've drafted your CV — take a look.",
        },
      ]);
      setIsDone(true);
    }
    ...
    <div className="rounded-(--radius-md) border border-(--border) bg-(--card) flex flex-col h-[560px] max-w-2xl">
      ...
      >
        {messages.map((m, i) => (
          ...
          >
            <div
              className={`w-7 h-7 rounded-(--radius-full) flex items-center justify-center shrink-0 ${
                m.role === "user"
            ...
            <div
              className={`rounded-(--radius-md) px-3 py-2 text-(--body-sm) max-w-[80%] ${
                m.role === "user"
